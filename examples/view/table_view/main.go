...
func newDemo() *qt.QWidget {
	...
	tableView := widgets.NewTableWidget(w)
	...
	for i, songInfo := range songInfos {
		for j := 0; j < 5; j++ {
			tableView.SetItem(i, j, qt.NewQTableWidgetItem2(songInfo[j]))
		}
	}

	tableView.VerticalHeader().Hide()
	tableView.SetHorizontalHeaderLabels([]string{"Title", "Artist", "Album", "Year", "Duration"})
	tableView.ResizeColumnsToContents()

	hBoxLayout.SetContentsMargins(50, 30, 50, 30)
	hBoxLayout.AddWidget(tableView.QWidget)
	w.Resize(735, 760)
	return w
}
